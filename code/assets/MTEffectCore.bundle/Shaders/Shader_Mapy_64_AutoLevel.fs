...
uniform highp vec3 levelCurveCoef;
...
void main() {
    ...
    highp float grayColor = dot(color, vec3(0.2989, 0.5870, 0.1140));

    highp float curveColor = levelCurveCoef.r * grayColor* grayColor + levelCurveCoef.g * grayColor + levelCurveCoef.b;
    highp float brightVal = curveColor/(grayColor+0.001);
    color = clamp(color*brightVal,0.0,1.0);
