...
vec3 rgb2xyz( vec3 rgbcolor ) {
	vec3 xyzcolor=vec3((( rgbcolor.r > 0.04045 ) ? pow( ( rgbcolor.r + 0.055 ) / 1.055, 2.4 ) : rgbcolor.r / 12.92), ( ( rgbcolor.g > 0.04045 ) ? pow( ( rgbcolor.g + 0.055 ) / 1.055, 2.4 ) : rgbcolor.g / 12.92), (( rgbcolor.b > 0.04045 ) ? pow( ( rgbcolor.b + 0.055 ) / 1.055, 2.4 ) : rgbcolor.b / 12.92));
...
void main(void) {
	...
	vec2 uv = gl_FragCoord.xy / size; vec2 d = 1.0 / size;
	vec3 c = texture2D(inputImageTexture0, uv).xyz;
	...
	if(mode==0){
		...
		float d = distance(texcoordOut, vec2(vignetteCenter.x, vignetteCenter.y));
